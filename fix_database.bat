...
python manage.py shell -c "from chatbot.models import KnowBase; deleted = KnowBase.objects.filter(title__icontains='pantip').delete(); print(f'Deleted {deleted[0]} records')"
echo.
